...
- **免費體驗**：無需登入即可使用一鍵生成功能
- **推薦邀請碼**：邀請好友獲得額度獎勵
  - 基礎獎勵：每邀請 1 人獲得 +50 次本月額度（最多 +500 次）
  - 額外獎勵：邀請 10 人或好友付款，可獲得 +500 次本月額度（二擇一）
  - 總計最多可獲得 +1000 次本月額度
- **實戰指南**：12 篇專業文章
- **幫助中心**：完整的 FAQ 和支援系統
- **訂閱付款**：綠界金流整合，支援月繳/年繳
- **PWA 支援**：支援安裝為手機 App，提供原生體驗
...
### 免費版
- **價格**：完全免費
- **使用次數**：每日 10 次 / 每月 100 次
- **特色**：適合試用和體驗
- **功能**：所有核心功能 + BYOK 支援（需綁定自己的 AI 金鑰）
- **限制**：平台不提供備用配額、不支援高品質模式、不支援批次生成
...
- **表格響應式**：在 PWA/手機版中，表格支援左右滑動，避免垂直擠壓
- **PWA 對話框優化**：輸入框在 PWA 模式下使用 fixed 定位，避免被鍵盤遮擋
...
- **前端用量檢查**：在發送請求前檢查今日/本月用量是否已達上限
- **即時更新**：API 調用成功後自動更新用量狀態
- **友好提示**：用量達上限時顯示明確的錯誤訊息和升級方案按鈕
- **錯誤區分**：區分「用量已達上限」和「API 配額用盡」兩種情況，提供不同的處理建議
...
- ✅ **表格響應式優化**：PWA/手機版表格支援左右滑動，避免垂直擠壓
- ✅ **PWA 對話框優化**：輸入框在 PWA 模式下固定定位，避免被鍵盤遮擋
- ✅ **Toast 通知優化**：深色模式下按鈕文字改為黑色，提升可讀性

### 🔧 功能增強
- ✅ **訂閱方案更新**：新增免費版，支援 Lite、Pro、Max 四種方案
- ✅ **Premium 模式**：Pro 和 Max 方案支援高品質模式
- ✅ **BYOK 支援**：所有方案都支援使用自己的 AI 金鑰
- ✅ **用量檢查機制**：前端用量檢查，用量達上限時顯示友好提示
- ✅ **推薦制度更新**：獎勵從「免費使用時間」改為「本月額度次數」
  - 基礎獎勵：每邀請 1 人 +50 次（最多 +500 次）
  - 額外獎勵：邀請 10 人或好友付款 +500 次（二擇一）
  - 總計最多 +1000 次本月額度

### 🐛 錯誤修復
- ✅ **Mode3 卡住問題**：修復 `checkUsageLimit` 函數未定義導致的錯誤
- ✅ **用量上限提示**：區分「用量已達上限」和「API 配額用盡」，提供不同的處理建議
- ✅ **14 天規劃分類**：改進分類邏輯，確保 14 天規劃正確保存到對應標籤
