...
- [x] **設定（Settings）**：LLM API Key 管理、數據匯出
...
- [x] **統計顯示**：成功邀請數、累積獎勵
- [x] **獎勵機制說明**：
  - 每成功邀請一位好友註冊 → 7 天免費試用延長（無限制）
  - **二選一獎勵（擇一發放，先達到條件者優先）：**
    - 好友完成首次訂閱 → 額外 30 天使用期限（每個被推薦用戶只能觸發一次）
    - 累積邀請 5 位付費用戶（必須完成付費訂閱，月付或年付均可）→ 1 個月免費使用（每個推薦人只能獲得一次）
...
#### 2.6 個人資料頁面 ✅
- [x] **基本資訊**：用戶資訊顯示與編輯
- [x] **創作者資訊**：平台帳號、粉絲數、內容類型
- [x] **偏好設定**：AI 語氣、語言、影片長度、主題類別
- [x] **帳務資訊**：訂單記錄、授權狀態
...
### 1. 推薦邀請碼系統（待實現）
- [ ] 後端數據庫表結構（referral_codes, referral_records, referral_rewards）
- [ ] 推薦碼生成 API
- [ ] 推薦統計 API
- [ ] 獎勵發放機制
- [ ] 通知系統（Toast、站內通知、Email）

詳細實現指南請參考：`使用統計價值與推薦邀請碼機制報告.md`

### 2. 使用統計優化（建議）
- [ ] 增加更多視覺化圖表（圓餅圖、柱狀圖、熱力圖）
- [ ] 對比分析（週期對比、成長率）
- [ ] 個人化洞察（最佳創作時段、最常用功能）
- [ ] 導出功能（PDF 報告、CSV 匯出）
