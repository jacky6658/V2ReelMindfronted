...
def validate_plan_type(plan: str) -> bool:
    ...
    Returns:
        是否有效
    """
    return plan in ("yearly", "lifetime")
...
"""
    ...
    async def save_ip_planning_result(request: Request, current_user_id: Optional[str] = Depends(get_current_user)):
        ...
        try:
            ...
            # 檢查 IP 人設規劃權限（只有永久使用方案才有）
            if not check_ip_planning_permission(user_id):
                return JSONResponse({
                    "error": "只有永久使用方案用戶可以使用 IP 人設規劃功能",
    ...
    async def get_user_activities(admin_user: str = Depends(get_admin_user)):
        ...
        try:
            ...
            print(f"ERROR: 獲取用戶活動失敗: {e}", exc_info=True)
            if conn:
                try:
                    conn.close()
                except:
                    pass
            return JSONResponse({"error": f"獲取活動失敗: {str(e)}"}, status_code=500)
...
<body>
    ...
    async def verify_license_webhook(request: Request):
        ...
        try:
            ...
            # 保留 "trial" 類型，但主要處理 "yearly" 和 "lifetime"
            ...
            # 計算授權到期日
            # 試用期邏輯：當 amount 為 0 時，即使是 yearly 或 lifetime，也設為 7 天試用期
            # 這樣用戶可以獲得完整權限（yearly/lifetime tier），但只有 7 天有效期
            if plan_type == "lifetime":
                if amount == 0:
                    # 試用期：7 天（但 tier 仍然是 lifetime，獲得完整權限）
                    days = 7
                    license_expires_at = get_taiwan_time() + timedelta(days=days)
                else:
                    # 真正的永久方案：設為 2099-12-31（表示永久有效）
                ...
                if amount == 0:
                    # 試用期：7 天（但 tier 仍然是 yearly，獲得完整權限）
                    days = 7
                else:
                    # 真正的年費方案：365 天
                    days = 365
                ...
                try:
                    ...
                    if use_postgresql:
                        ...
                        cursor.execute("""
                            ...
                            WHERE user_id = %s AND tier = 'yearly'
                        """, (user_id,))
                    else:
                        # 更新 user_auth 表
                        cursor.execute("""
...
ReelMind 團隊
                    ...
                    error_msg = f"取消用戶 {user_id} 訂閱資格時發生錯誤: {str(e)}"
                    logger.error(error_msg, exc_info=True)
            ...
            for row in expiring_subscriptions:
                ...
                try:
                    ...
                    # 永久使用升級連結（主推）- 直接呼叫升級 API
                    upgrade_url = f"{backend_base_url}/api/payment/upgrade-to-lifetime"
                    
                    # 計算成本比較（FOMO 元素）
                    yearly_4years = 4788 * 4  # 4年年費總額
                    lifetime_price = 9900  # 永久使用價格
                    savings = yearly_4years - lifetime_price  # 節省金額
                    savings_percent = int((savings / yearly_4years) * 100)  # 節省百分比
                    
                    # 發送 Email 通知（FOMO 風格，主推永久使用）
                    email_sent = False
                    if email and SMTP_ENABLED:
                        email_subject = f"【限時優惠】您的訂閱即將到期，升級永久使用享最大優惠！"
                        email_body = f"""
親愛的 {name}，

⚠️ 重要通知：您的 ReelMind 年費訂閱將在 {expires_at.strftime('%Y年%m月%d日')} 到期。

🎯 現在是升級永久使用的最佳時機！

您知道嗎？如果您每年續費，4 年就要花費 NT$ {yearly_4years:,}
但現在升級永久使用，只需 NT$ {lifetime_price:,}，一次購買，終身使用！

💎 永久使用的優勢：
✅ 一次付款，永久擁有
✅ 無需每年續費，省時省心
✅ 未來所有功能更新，自動享有
✅ 長期使用成本最低（比年費省 {savings_percent}% 以上）

🔥 限時優惠（僅限到期前 3 天）：
升級永久使用，立即解鎖所有功能，再也不用擔心到期問題！

【立即升級永久使用】← 點擊這裡（系統會自動計算差價）
{upgrade_url}
                            ...
                            <div style="max-width: 600px; margin: 0 auto; background: white; padding: 0;">
                                ...
                                <div style="padding: 30px 20px;">
                                    ...
                                    <div style="background: linear-gradient(135deg, #FEF3C7 0%, #FDE68A 100%); border-left: 4px solid #F59E0B; padding: 20px; border-radius: 8px; margin: 20px 0;">
                                        <div style="font-size: 20px; font-weight: 900; color: #92400E; margin-bottom: 12px;">💡 升級永久使用，現在最划算！</div>
                                        <div style="color: #78350F; line-height: 1.8;">
                                            <p style="margin: 8px 0;"><strong>您知道嗎？</strong></p>
                                            <p style="margin: 8px 0;">• 年費續費 4 年 = <span style="text-decoration: line-through; color: #991B1B;">NT$ {yearly_4years:,}</span></p>
                                            <p style="margin: 8px 0;">• 永久使用 = <strong style="color: #DC2626; font-size: 18px;">NT$ {lifetime_price:,}</strong>（省 {savings_percent}%！）</p>
                                            <p style="margin: 8px 0; font-weight: 600;">一次購買，終身使用，再也不用擔心到期問題！</p>
                                        </div>
                                    </div>
                                    
                                    <!-- 永久使用優勢 -->
                                    <div style="background: #EFF6FF; border: 2px solid #2563EB; border-radius: 12px; padding: 20px; margin: 20px 0;">
                                        <h3 style="color: #1E40AF; margin-top: 0; font-size: 20px; text-align: center;">💎 永久使用的優勢</h3>
                                        <div style="display: grid; gap: 12px; margin-top: 16px;">
                                            <div style="display: flex; align-items: start; gap: 10px;">
                                                <span style="font-size: 24px;">✅</span>
                                                <div>
                                                    <strong>一次付款，永久擁有</strong>
                                                    <p style="margin: 4px 0 0 0; color: #64748B; font-size: 14px;">無需每年續費，省時省心</p>
                                                </div>
                                            </div>
                                            <div style="display: flex; align-items: start; gap: 10px;">
                                                <span style="font-size: 24px;">✅</span>
                                                <div>
                                                    <strong>未來所有功能更新，自動享有</strong>
                                                    <p style="margin: 4px 0 0 0; color: #64748B; font-size: 14px;">AI 功能持續升級，您都能使用</p>
                                                </div>
                                            </div>
                                            <div style="display: flex; align-items: start; gap: 10px;">
                                                <span style="font-size: 24px;">✅</span>
                                                <div>
                                                    <strong>長期使用成本最低</strong>
                                                    <p style="margin: 4px 0 0 0; color: #64748B; font-size: 14px;">比年費方案省 {savings_percent}% 以上</p>
                                                </div>
                                            </div>
                                            <div style="display: flex; align-items: start; gap: 10px;">
                                                <span style="font-size: 24px;">✅</span>
                                                <div>
                                                    <strong>無到期壓力</strong>
                                                    <p style="margin: 4px 0 0 0; color: #64748B; font-size: 14px;">再也不用擔心忘記續費</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- 主要 CTA：升級永久使用 -->
                                    <div style="text-align: center; margin: 30px 0;">
                                        <a href="{upgrade_url}" style="display: inline-block; padding: 16px 32px; background: linear-gradient(135deg, #2563EB 0%, #1D4ED8 100%); color: white; text-decoration: none; border-radius: 8px; font-weight: 900; font-size: 18px; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.4);">🚀 立即升級永久使用（系統自動計算差價）</a>
                                        <p style="margin: 8px 0 0 0; color: #64748B; font-size: 14px;">根據您的剩餘天數自動計算最優惠價格</p>
                                    </div>
                                    
                                    <!-- 限時提醒 -->
                                    <div style="background: #FEF2F2; border-left: 4px solid #EF4444; padding: 15px; border-radius: 8px; margin: 20px 0;">
                                        <p style="margin: 0; color: #991B1B; font-weight: 600; text-align: center;">
                                            ⚠️ 限時優惠：僅限到期前 3 天內升級，錯過就要等明年了！
                                        </p>
                                    </div>
                                    
                                    <!-- 分隔線 -->
                                    <div style="text-align: center; margin: 30px 0; color: #94A3B8; position: relative;">
                                        <div style="position: absolute; top: 50%; left: 0; right: 0; height: 1px; background: #E5E7EB;"></div>
                                        <span style="background: white; padding: 0 15px; position: relative;">或</span>
                                    </div>
                                    
                                    <!-- 次要選項：續費年費 -->
                                    <div style="background: #F9FAFB; border: 1px solid #E5E7EB; border-radius: 8px; padding: 20px; margin: 20px 0;">
                                        <h3 style="margin-top: 0; color: #64748B; font-size: 16px;">也可以選擇續費年費方案</h3>
                                        <div style="background: white; padding: 15px; border-radius: 6px; margin: 12px 0;">
                                            <p style="margin: 4px 0;"><strong>訂單號：</strong>{trade_no}</p>
                                            <p style="margin: 4px 0;"><strong>方案：</strong>年費方案</p>
                                            <p style="margin: 4px 0;"><strong>金額：</strong>NT$ {amount:,}</p>
                                            <p style="margin: 4px 0;"><strong>新到期日：</strong>{new_expires_at.strftime('%Y年%m月%d日')}</p>
                                        </div>
                                        <div style="text-align: center;">
                                            <a href="{payment_url}" style="display: inline-block; padding: 12px 24px; background: #64748B; color: white; text-decoration: none; border-radius: 6px; font-weight: 600;">續費年費方案</a>
                                        </div>
                                    </div>
